...
dy= (sigma.*Cl_alpha.*((aa.*db)+(bb.*da)))-dc; %derivata di y

i=1;

epsi= 1.e-5; % nel listato fortran vi è un termine 'err' che ...
             % non è specificato viene preso dall'esterno 
if abs(y./dy) < epsi
w_t_B= w_t_B-(y./dy);
else
    while ((abs(y./dy)> epsi) | i==max_iter+1)
