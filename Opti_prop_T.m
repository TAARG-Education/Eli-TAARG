...
function [r_adim,chi,a_corr,a_first_corr,dCt_dradim,dCp_dradim,Cp] = Opti_prop_T(N_blade,R_hub,R_tip,n_rpm,V_inf,Ct,h)
%% CONVERSIONS
%{
...
%}
dT_dradim = N_blade*rho*n_rads*(R_tip^2)*r_adim.*(1-a_first).*GAMMA;
dCt_dradim= dT_dradim/(rho*(n_rps^2)*(D^4));
Ct_new    = trapz(r_adim,dCt_dradim);
error_new = Ct_new - Ct;
